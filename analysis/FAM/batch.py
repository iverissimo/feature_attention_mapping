    ...
    def copy_fit_deriv(self, use_rsync = False, task = 'pRF', use_fs_label = False):
        ...
        if self.group_bool:
            if task == 'pRF':
                ...
                if use_rsync:
                    fit_cmd = """mkdir -p $TMPDIR/derivatives/{$FITFOLDER,$PRFFITFOLDER}/$SPACE/\nwait\n"""+ \
                        """mkdir -p $TMPDIR/derivatives/{$FITFOLDER,$PRFFITFOLDER}/$PRFSPACE/\nwait\n\n"""+ \
            ...
            if task == 'pRF':
                ...
                if use_rsync:
                    fit_cmd = """mkdir -p $TMPDIR/derivatives/{$FITFOLDER,$PRFFITFOLDER}/$SPACE/sub-$SJ_NR\nwait\n"""+ \
                        """mkdir -p $TMPDIR/derivatives/{$FITFOLDER,$PRFFITFOLDER}/$PRFSPACE/sub-$SJ_NR\nwait\n\n"""+ \
