...
export SUBJECTS_DIR=$TMPDIR/freesurfer

wait

#recon-all -autorecon2 -autorecon3 -s $SJ_NR
recon-all -all -s $SJ_NR

wait

rsync -chavzP $TMPDIR/freesurfer/ $DATADIR
...
keys2replace = {'$SJ_NR': str(sj).zfill(3),
                '$DATADIR': op.join(params['mri']['paths'][base_dir]['root'],'derivatives','freesurfer')
                 }
...
# run it
js_name = op.join(batch_dir, 'FREESURFER-' + str(sj).zfill(3) + '_FAM_recon.sh')
of = open(js_name, 'w')
