...
for blk in range(params['feature']['mini_blocks']): # for each miniblock
    ...
    cue_convolved = Parallel(n_jobs=16)(delayed(signal.fftconvolve)(cue_regs_upsampled[blk][vert], 
                                                hrf_oversampled[vert], 
                                                mode='full', 
                                                axes=(-1)) for _,vert in enumerate(tqdm(mask_ind)))

    # save convolved upsampled cue in array
    for i,ind in enumerate(mask_ind):
        cue_regs_upsampled[blk][ind] = np.array(cue_convolved)[i,:cue_regs_upsampled.shape[-1]]
